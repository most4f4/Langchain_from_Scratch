...
# -------------------------------------------------------------
# This script demonstrates different embedding models and their
# impact on creating vector stores for RAG applications.
# Each embedding model is used to create a separate vector store,
# which are then queried to compare results.
# so far we have used only OpenAIEmbeddings with model "text-embedding-3-small"
# which is a good general purpose embedding model and least expensive one
# but now we compare text-embedding-ada-002 from OpenAI which allows for more pages per dollar
# We also compare with Hugging Face embeddings which are free to use
# if you run them on your local machine (no API costs, but uses your compute resources)
# Note: The quality of embeddings may vary based on the model and use case.
# -------------------------------------------------------------
...
# Pricing: https://platform.openai.com/docs/guides/embeddings
